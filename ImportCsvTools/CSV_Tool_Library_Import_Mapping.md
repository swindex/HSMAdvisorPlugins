...
## Inherited Fields: InventoryItem (Tool inherits from InventoryItem)

`ToolDatabase.Tool` inherits from `HSMAdvisorDatabase.ToolDataBase.InventoryItem`, so all InventoryItem fields below are also valid `ToolField` targets in the mapping file. fileciteturn1file0

> **Autogenerated fields:** These are normally created/maintained by HSMAdvisor. You *can* map them, but most imports should **omit** them unless you explicitly want to override defaults.

| Field | Type | Required | Autogenerated | Description | Example Mapping |
|-------|------|----------|--------------|-------------|-----------------|
| isNew | bool | No | Yes | Runtime flag, defaults to false | `{"CsvColumn": "Is New", "ToolField": "isNew"}` |
| Guid | string | No | Yes | Convenience property; if empty it generates a new Guid and stores it in `_Guid` | `{"CsvColumn": "Guid", "ToolField": "Guid"}` |
| Library | string | No | No | Library name/group label for the item | `{"CsvColumn": "Library", "ToolField": "Library"}` |
| Comment | string | No | No | Free-form comment | `{"CsvColumn": "Comment", "ToolField": "Comment"}` |
| Brand_name | string | No | No | Brand/manufacturer name | `{"CsvColumn": "Brand", "ToolField": "Brand_name"}` |
| Series_name | string | No | No | Product series name | `{"CsvColumn": "Series", "ToolField": "Series_name"}` |
| Version | int | No | No | Convenience int wrapper over `_Version` | `{"CsvColumn": "Version", "ToolField": "Version"}` |
| Supplier | string | No | No | Supplier name | `{"CsvColumn": "Supplier", "ToolField": "Supplier"}` |
| Supplier_pid | string | No | No | Supplier part id / SKU | `{"CsvColumn": "Supplier PID", "ToolField": "Supplier_pid"}` |
| Product_link | string | No | No | URL to product page | `{"CsvColumn": "Product Link", "ToolField": "Product_link"}` |
| Product_link | string | No | No | URL to product page | `{"CsvColumn": "Product Link", "ToolField": "Product_link"}` |
| Create_date | DateTime | No | Yes | Auto-set to UTC now in constructor | `{"CsvColumn": "Create Date", "ToolField": "Create_date"}` |
| Update_date | DateTime | No | Yes | Auto-set to UTC now in constructor (and typically updated on save) | `{"CsvColumn": "Update Date", "ToolField": "Update_date"}` |
| Additional_info | string | No | No | Extra info blob / notes | `{"CsvColumn": "Additional Info", "ToolField": "Additional_info"}` |
| Location | string | No | No | Primary storage location | `{"CsvColumn": "Location", "ToolField": "Location"}` |
| Location_1 | string | No | No | Secondary location | `{"CsvColumn": "Location 1", "ToolField": "Location_1"}` |
| Location_2 | string | No | No | Tertiary location | `{"CsvColumn": "Location 2", "ToolField": "Location_2"}` |
| Location_3 | string | No | No | Quaternary location | `{"CsvColumn": "Location 3", "ToolField": "Location_3"}` |

### Notes for InventoryItem fields

- Prefer mapping **either** `Guid` **or** `_Guid` (not both). If you omit both, a new guid is generated automatically.
- If you omit `Create_date` / `Update_date`, the constructor defaults will be used.
- `Version` is a convenience wrapper around `_Version`. If your CSV stores version as a number, map to `Version`; if it stores it as text, map to `_Version`.
